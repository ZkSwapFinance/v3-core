...
/// @title Canonical ZF V3 factory
/// @notice Deploys ZF V3 pools and manages ownership and control over pool protocol fees
contract ZFV3Factory is IZFV3Factory {
    /// @inheritdoc IZFV3Factory
    ...
    constructor(address _poolDeployer) {
        ...
        feeAmountTickSpacing[400] = 4;
        feeAmountTickSpacingExtraInfo[400] = TickSpacingExtraInfo({whitelistRequested: false, enabled: true});
        emit FeeAmountEnabled(400, 4);
        emit FeeAmountExtraInfoUpdated(400, false, true);
        feeAmountTickSpacing[2000] = 20;
        feeAmountTickSpacingExtraInfo[2000] = TickSpacingExtraInfo({whitelistRequested: false, enabled: true});
        emit FeeAmountEnabled(2000, 20);
    ...
    function createPool(
        ...
        pool = IZFV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IZFV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IZFV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IZFV3Pool(pool).setLmPool(lmPool);
    }
}
