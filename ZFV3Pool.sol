...
import './interfaces/IZFV3Pool.sol';
...
import './interfaces/IZFV3PoolDeployer.sol';
import './interfaces/IZFV3Factory.sol';
import './interfaces/IERC20Minimal.sol';
import './interfaces/callback/IZFV3MintCallback.sol';
import './interfaces/callback/IZFV3SwapCallback.sol';
import './interfaces/callback/IZFV3FlashCallback.sol';

import './interfaces/IZFV3LmPool.sol';

contract ZFV3Pool is IZFV3Pool {
    ...
    /// @inheritdoc IZFV3PoolImmutables
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == IZFV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IZFV3PoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc IZFV3PoolDerivedState
    ...
    function mint(
        ...
        IZFV3MintCallback(msg.sender).zfV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IZFV3SwapCallback(msg.sender).zfV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        IZFV3FlashCallback(msg.sender).zfV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc IZFV3PoolOwnerActions
    function setFeeProtocol(uint32 feeProtocol0, uint32 feeProtocol1) external override lock onlyFactoryOrFactoryOwner {
        require(
            (feeProtocol0 == 0 || (feeProtocol0 >= 500 && feeProtocol0 <= 5000)) &&
            (feeProtocol1 == 0 || (feeProtocol1 >= 500 && feeProtocol1 <= 5000))
        );
